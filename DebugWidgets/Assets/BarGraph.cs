...
public class BarGraph : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        signedDisplay = this.Min < 0;
        if (signedDisplay)
        {
            BarTransform.localPosition = new Vector2(width / 2.0f, BarTransform.localPosition.y);
        }
    ...
    public void SetWidthPercent(float value, Color c)
    {
        ...
        var xscale = Max * value;
        if (signedDisplay) xscale *= 0.5f;
        BarTransform.localScale = new Vector2(xscale, BarTransform.localScale.y);
    ...
    public static BarGraph Find(string name, Vector2 position, float min, float max)
    {
        ...
        if (BarGraphTable.TryGetValue(name, out var graph))
        {
            return graph;
        }
